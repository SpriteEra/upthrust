            ...
            <div className="mx-auto max-xl:flex xl:grid grid-cols-6 max-xl:overflow-x-auto w-full max-xl:h-100  hide-scrollbar">
                ...
                {steps.map((item, index) => (
                    ...
                    >
                        ...
                        <span className={`mb-6 lg:text-xs 3xl:text-base text-black/70 rounded-full px-3.75 py-2.5 lg:px-3 3xl:px-3.75 lg:py-1 3xl:py-2.5 ${index === 0 ? "bg-[#FF3B00]  text-white" : ""}`}>
                            {index === 0 ? 'Free' : ''} {`STEP #${index + 1} `}
                        </span>
                        <div className={`w-full ${item.offset}`}>
                            <div className="w-full">
                                <div className="absolute top-0 -right-[0px] h-65 3xl:h-85 w-px bg-black/30" />
                                {
                                    index === 0 &&
                                    <div className="absolute top-0 -left-[0px] h-65 3xl:h-85 w-px bg-black/30" />
                                }
                                {/* Step label */}

                                {/* Title */}
                                <p className="text-center text-lg lg:text-base 3xl:text-lg mb-4 3xl:mb-6 leading-[28px] lg:leading-[18px] 3xl:leading-6 text-gray-800 whitespace-pre-line h-14 lg:h-9">
                                    {item.title}
                                </p>
                            ...
                            <div className="relative w-full flex justify-center group">
                                ...
                                {/* Button */}
                                <div className="w-full bg-black rounded-full flex items-center justify-center py-1 3xl:py-2.5 cursor-pointer" >
                                    <span className="relative text-black bg-white rounded-full p-1.5 size-7 2xl:size-8 3xl:size-6 flex items-center justify-center">
                                        {/* Plus icon (default) */}
                                        <Plus
                                            strokeWidth={3}
