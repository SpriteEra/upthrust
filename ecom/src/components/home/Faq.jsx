...
export default function FAQ({ faqData = faqs, actionCss = { active: "bg-[#ff4d00] text-white", default: "bg-[#ff4d00] text-white" }, shadow = false }) {
    ...
    const renderFAQ = (faq, index) => (
        <div
            ...
            className={`bg-white rounded-xl 3xl:rounded-2xl overflow-hidden transition-all duration-300 cursor-pointer ${!shadow ? "border" : ""} border-[#e1e1e1] relative min-h-22.5`}
            style={
                shadow
                    ? {
                        boxShadow:
            ...
            <div className="p-4 md:p-6 md:px-8 max-w-[87%]">
                <div className="flex items-start justify-between gap-4 ">
                    ...
                    <button
                        ...
                        className={`absolute top-5 3xl:top-7 right-5 3xl:right-7 size-6 3xl:size-8 rounded-lg flex items-center justify-center shrink-0 transition-all duration-300 cursor-pointer ${openIndexes === index ? actionCss.active : actionCss.default
                            }`}
                    >
                        {openIndexes === index ? (
