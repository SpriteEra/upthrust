...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (!panel || !navLogo || !centerLogo || !scrollBanner || !bgVideo) return;
      ...
      // ── KEY FIX: measure positions while panel is in its FINAL expanded state ──
      const getNavLogoTarget = () => {
        // 1. Reset any previous logo transforms
        gsap.set(centerLogo, { x: 0, y: 0, scale: 1 });

        // 2. Snapshot current panel inline styles so we can revert
        const prevWidth = panel.style.width;
        const prevHeight = panel.style.height;
        const prevBorderRadius = panel.style.borderRadius;

        // 3. Force panel into its final expanded state
        gsap.set(panel, { width: '100vw', height: '100vh', borderRadius: 0 });

        // 4. Force a reflow so getBoundingClientRect reflects the new layout
        panel.getBoundingClientRect();

        // 5. Measure real positions now that everything is where it'll end up
        const logoRect = centerLogo.getBoundingClientRect();
        const navLogoRect = navLogo.getBoundingClientRect();
        ...
        // 6. Revert panel back to its initial state
        gsap.set(panel, {
          width: prevWidth || '62vw',
          height: prevHeight || '72vh',
      ...
      // Hide nav logo placeholder (it's only a measurement target)
      gsap.set(navLogo, { opacity: 0 });
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {/*
                Width here should match your logo's rendered width at 32px height.
                If your logo image is ~3.5:1 ratio → 32px tall ≈ 112px wide.
                Adjust until the logo lands perfectly centered in the nav.
              */}
              <div style={{ width: '112px', height: '32px' }} />
            </div>
