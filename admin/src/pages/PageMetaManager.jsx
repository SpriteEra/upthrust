...
import {
    ...
    Search, AlertTriangle, Globe, Link2,
    Image, ChevronDown, ChevronUp, Eye, EyeOff,
    FileSearch, Tag, Share2,
    TextWrap,
} from "lucide-react";
import {
    getAllPageMetaAPI,
...
const Field = ({ label, hint, children, charCount }) => (
    <div>
        ...
        <div className="flex justify-between items-center mt-1">
            ...
            {charCount !== undefined && (
                <p className={`text-[11px] ml-auto font-medium ${charCount > 160 ? "text-red-400" : charCount > 120 ? "text-amber-400" : "text-slate-400"}`}>
                    {charCount} chars
                </p>
            )}
        </div>
...
const Divider = ({ label, icon: Icon, color = "text-slate-400" }) => (
    <div className="flex items-center gap-2 my-2">
        ...
        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 flex items-center gap-1.5 ${color}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                    ...
                    <Field label="Meta Title" hint="Browser tab + Google result title" charCount={form.title.length}>
                        <Input
                    ...
                    <Field label="Meta Description" hint="Google snippet — aim for 140–160 chars" charCount={form.description.length}>
                        <Textarea
                            rows={3}
                    ...
                    <Field label="Canonical URL" hint="Leave empty to use slug automatically">
                        <div className="relative">
                            <Link2 size={14} className="absolute left-3 top-3 text-slate-400" />
                            <Input
                                className="pl-8"
                    ...
                    <div className="flex gap-6">
                        {[
                            { key: "robots_index", label: "index", hint: "Allow Google to index" },
                            { key: "robots_follow", label: "follow", hint: "Follow links on page" },
                        ].map(({ key, label, hint }) => (
                    ...
                    <Field label="OG Image Path" hint="e.g. /ogimage/google-og.png  (1200×630)">
                        <div className="relative">
                            <Image size={14} className="absolute left-3 top-3 text-slate-400" />
                            <Input
                                className="pl-8"
                    ...
                    {/* ── Twitter ── */}
                    <Divider label="Twitter / X Card" icon={TextWrap} color="text-slate-600" />
                    ...
                    <Field label="Twitter Description" charCount={form.tw_description.length}>
                        <Textarea
                            rows={2}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <h3 className="font-semibold text-slate-800 mb-1">Delete Metadata</h3>
                        <p className="text-sm text-slate-500">
                            Delete metadata for <span className="font-medium text-slate-700">"{meta.label}"</span>? This cannot be undone.
                        </p>
                    </div>
                </div>
            ...
            <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                <td className="px-4 py-3">
                    <button onClick={() => setExpanded((v) => !v)} className="text-left w-full">
                        <p className="text-sm font-medium text-slate-800 group-hover:underline underline-offset-2">{meta.label}</p>
                        <code className="text-[11px] text-slate-400 font-mono">/{meta.slug}</code>
                    </button>
                </td>
                <td className="px-4 py-3 max-w-[220px]">
                    <p className="text-sm text-slate-600 truncate">{meta.title || <span className="text-slate-300 italic text-xs">—</span>}</p>
                </td>
                <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-slate-100 text-slate-600">
                            <Tag size={9} /> {Array.isArray(meta.keywords) ? meta.keywords.length : 0} kw
                        </span>
                        ...
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${meta.robots?.index !== false ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
                            {meta.robots?.index !== false ? <Eye size={9} /> : <EyeOff size={9} />}
                            {robotsLabel}
                        </span>
                    </div>
                </td>
                <td className="px-4 py-3 text-xs text-slate-400 whitespace-nowrap">
                    {new Date(meta.updatedAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}
                </td>
                <td className="px-4 py-3">
                    <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => setExpanded((v) => !v)} className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors">
                            {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                        </button>
                        <button onClick={() => onEdit(meta)} className="p-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors">
                            <Pencil size={14} />
                        </button>
                        <button onClick={() => onDelete(meta)} className="p-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition-colors">
                            <Trash2 size={14} />
                        </button>
                    </div>
                </td>
                ...
                <tr className="border-b border-slate-100 bg-slate-50">
                    <td colSpan={5} className="px-6 py-5">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            ...
                            <div>
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 p-4">
                                    <p className="text-[13px] text-blue-600 font-medium truncate">
                                        {meta.title || <span className="text-slate-300 italic">No title</span>}
                                    </p>
                                    <p className="text-[11px] text-green-700 mt-0.5">upthrust.agency/{meta.slug}</p>
                                    <p className="text-[12px] text-slate-500 mt-1 line-clamp-2">
                                        {meta.description || <span className="italic text-slate-300">No description</span>}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-1.5 text-[12px]">
                                    <div><span className="font-semibold text-slate-500">title: </span><span className="text-slate-700">{meta.openGraph?.title || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">desc: </span><span className="text-slate-700 line-clamp-2">{meta.openGraph?.description || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">url: </span><span className="text-slate-700 truncate block">{meta.openGraph?.url || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">siteName: </span><span className="text-slate-700">{meta.openGraph?.siteName || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">image: </span><code className="text-slate-600 text-[11px]">{meta.openGraph?.image || "—"}</code></div>
                                    <div><span className="font-semibold text-slate-500">type: </span><span className="text-slate-700">{meta.openGraph?.type || "—"}</span></div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-1.5 text-[12px]">
                                    <div><span className="font-semibold text-slate-500">card: </span><span className="text-slate-700">{meta.twitter?.card || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">title: </span><span className="text-slate-700">{meta.twitter?.title || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">desc: </span><span className="text-slate-700 line-clamp-2">{meta.twitter?.description || "—"}</span></div>
                                    <div><span className="font-semibold text-slate-500">image: </span><code className="text-slate-600 text-[11px]">{meta.twitter?.image || "—"}</code></div>
                                </div>
                            </div>
                        ...
                        {meta.keywords?.length > 0 && (
                            <div className="mt-4">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Keywords</p>
                                <div className="flex flex-wrap gap-1.5">
                                    {meta.keywords.map((k, i) => (
                                        <span key={i} className="px-2.5 py-0.5 bg-white border border-slate-200 text-slate-600 text-[11px] rounded-full">{k}</span>
                                    ))}
                                </div>
                            </div>
                        )}
...
export default function PageMetaManager() {
    ...
    const filtered = metas.filter((m) => {
        ...
        return !q || m.slug.includes(q) || m.label.toLowerCase().includes(q) || m.title?.toLowerCase().includes(q);
    });
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
                {[
                    { label: "Total Pages",   value: metas.length,                                        color: "bg-slate-100 text-slate-700" },
                    { label: "With OG Image", value: metas.filter((m) => !!m.openGraph?.image).length,    color: "bg-sky-50 text-sky-700" },
                    { label: "noindex",       value: metas.filter((m) => m.robots?.index === false).length, color: "bg-red-50 text-red-600" },
                ].map((s) => (
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                        ...
                        <p className="text-xs mt-1">
                            {metas.length > 0 ? "Try adjusting your search" : 'Click "New Metadata" to add your first entry'}
                        </p>
                    </div>
                ) : (
                    <table className="w-full">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Label / Slug</th>
                                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Meta Title</th>
            ...
            {modal?.type === "create" && (
                <MetaModal mode="create" onClose={() => setModal(null)} onSaved={handleSaved} />
            )}
            {modal?.type === "edit" && (
                <MetaModal mode="edit" initial={modal.meta} onClose={() => setModal(null)} onSaved={handleSaved} />
            )}
            {modal?.type === "delete" && (
                <DeleteModal meta={modal.meta} onClose={() => setModal(null)} onDeleted={handleDeleted} />
            )}
        </div>
    );
}